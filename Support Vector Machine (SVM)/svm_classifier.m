...
% %
linmodel = fitcsvm(x2mat, yvec, 'Standardize',true,'Solver','L1QP'); %Set standardize to true.
...
% %
cvmodel = crossval(linmodel,'kfold',5); %Perform 5-fold cross validation
...
% %
[lbls,scores] = predict(linmodel,x2mat); %Need the label variable for the confusion matrix.
scores = scores(:,2);
% Compute the confusion matrix
% % 
% % STUDENT CODE: REPLACE THIS LINE WITH THE CONFUSION MATRIX
% %
cmat = confusionmat(yvec,lbls,'Order',[1 -1]);

% Compute the ROC curve and its AUC
% % 
% % STUDENT CODE: USE "PERFCURVE" TO GENERATE THESE VARIABLES
% %
[xroc,yroc,~,auc] = perfcurve(yvec,scores,1);
disp(sprintf('\nAccuracy from 5-fold cross-validation is = % 3.2f. \n\nConfusion matrix is:', acc));
disp(cmat);
cmatacc = 100*(cmat(1,1)+cmat(2,2))/(sum(sum(cmat)));
disp(sprintf('The accuracy from the confusion matrix is: %3.2f',cmatacc));
% Plot the ROC curve
figure(2);
