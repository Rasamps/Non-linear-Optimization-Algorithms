function a4q2
% Sample code for Assignment #4, Question #2
...
% % NOTE: DIFFERENT METHOD THAN SPECIFIED IN QUESTION #1
% %
[U,S,V] = svd(a4data5d(:,1:5),'econ');
x2d = U(:,1:2)*S(:,1:2)';
xmat = 10*(zscore(x2d(:,1:2))/sqrt(size(x2d,1)-1))';
...
% Flag to control whether to use a Gaussian SVM or a linear SVM
% %
% % STUDENT CODE: MODIFY THIS FLAG
% %
do_gaussian = 1;
...
disp(sprintf('A4Q2: classified %0.2f%% correctly', 100*cval));
end
...
for jx = 1:xm
    ...
    for ix = 1:sm
        ...
        svec = smatT(ix, :)';
        ylabel = svmStruct.SupportVectorLabels(ix);
        alpha = svmStruct.Alpha(ix);
        thisScore = alpha*ylabel*(exp(-gamma*(norm(thisX - svec)^2)));
        score = score + thisScore;
    end
    scorevec(jx) = score + bias;
